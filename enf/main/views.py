...
class CatalogView(TemplateView):
    ...
    FILTER_MAPPING = {
        'author': lambda queryset, value: queryset.filter(author=value),
        'part': lambda queryset, value: queryset.filter(part=value),
        'grade': lambda queryset, value: queryset.filter(grade=value),
    }
...
def get_context_data(self, **kwargs):
    ...
    products = Product.objects.all().order_by('-created_at')  #quest
    ...
    for param, filter_func in self.FILTER_MAPPING.items():
        ...
        if value:
            products = filter_func(products, value)#22/09
...
class ProductDetailView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        raise TemplateResponse(request, self.template_name, context) #raise